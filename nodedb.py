...
class NodeDB:
    ...
    def count_clients(self):
        ...
        already_counted_client_macs = []

        for link in self._links:
            try:
                source_node = self.maybe_node_by_mac([link.source.interface])
                target_node = self.maybe_node_by_mac([link.target.interface])

                if source_node.flags['client']:
                    client = source_node
                    node = target_node
                elif target_node.flags['client']:
                    client = target_node
                    node = source_node
                else:
                    continue
                ...
                is_duplicate = False
                for client_mac in client.macs:
                    if client_mac in already_counted_client_macs:
                        is_duplicate = True
                    else:
                        already_counted_client_macs.append(client_mac)
